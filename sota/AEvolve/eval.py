"""
DYNAMIC Evaluation script for LLMGE

Evaluates tensor decomposition models (like the seed model) and returns fitness metrics.

Fitness Objectives:
1. RANK RATIO: compares best rank found vs trivial rank (ex: 24/27) - MINIMIZE
2. Incorrect matrix entries - MINIMIZE
3. Number of basis multiplication tests passed - MAXIMIZE
4. Number of random multiplication tests passed - MAXIMIZE
5. Median runtime in NS (nanoseconds) per multiplication test - MINIMIZE
6. Number of additions (sparsity of factors) - MINIMIZE
        ...
        for R in range(size_cfg['starting_R'], size_cfg['min_R'] - 1, -1):
            ...
            try:
                ...
                if is_valid:
                    ...
                    print(f"  [R={R}] INVALID (entries_wrong={fundamental_check:.4f} - should be 0, basis={basis_score:.4f}, "
